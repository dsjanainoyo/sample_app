...
describe '投稿のテスト' do
    ...
    descrice 'トップ画面(top_path)への遷移' do
        before do
            visit top_path
        end
        context '表示の確認' do
    ...
    describe '投稿画面のテスト' do
        ...
        context '表示の確認' do
            it 'new_list_pathが"/lists/new"であるか' do
                expext(current_path).to eq('/lists/new')
            end
    ...
    describe '詳細画面のテスト' do
        ...
        context 'list削除のテスト' do
            it 'listの削除' do
                ...
                destroy_link.click
                expext{ list.destroy }.to change{ List.count }.by(-1)
            end
        end
    end
    ...
    describe '編集画面のテスト' do
        ...
        context '表示の確認' do
            ...
            it '保存ボタンが表示されている' do
                expext(page).to have_button '保存'
            end
        end
        context '更新処理に関するテスト' do
            it '更新後のリダイレクト先は正しいか' do
                fill_in 'list[title]', with: Faker::Lorem.character(number:10)
                fill_in 'list[body]', with: Faker::Lorem.character(number:30)
                click_button '保存'
